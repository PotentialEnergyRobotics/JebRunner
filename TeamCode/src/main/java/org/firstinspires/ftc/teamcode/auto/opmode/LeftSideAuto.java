...
public class LeftSideAuto extends OpMode {
    ...
    private final double SIG_DIST_COUNT = 1;
    ...
    public void init() {
        ...
        trajectories.add(drive.trajectorySequenceBuilder(firstPoleStartPose)
                ...
                .splineToSplineHeading(new Pose2d(-55 * X_MOD,-15, Math.toRadians(X_MOD * (225 + HEAD_MOD))), Math.toRadians(X_MOD * (180 + TAN_MOD)))
                .build());

        // Trajectory from cone stack to centerline tall pole
        Pose2d coneStackStartPose = new Pose2d(-61 * X_MOD,-11, Math.toRadians(X_MOD * (225 + HEAD_MOD)));
        trajectories.add(drive.trajectorySequenceBuilder(coneStackStartPose)
                .setTangent(Math.toRadians(X_MOD * (0 + TAN_MOD)))
                .waitSeconds(1)
                .splineToSplineHeading(new Pose2d(-24 * X_MOD + POLE_OFFSET,-14, Math.toRadians(X_MOD * (135 + HEAD_MOD))), Math.toRadians(X_MOD * (0 + TAN_MOD)))
                .build());
        ...
        motions.add(new Motion() {
            ...
            public boolean isEnd() {
                ...
                return distCount > SIG_DIST_COUNT;
            }
        ...
        // Align with cone stack using front distance sensor
        motions.add(new Motion() {
            @Override
            public boolean isEnd() {
                dist = drive.distanceSensorFront.getDistance(DistanceUnit.CM);
                if (dist < minDist) minDist = dist;
                return dist > minDist + 2;
            }

            @Override
            public void init() { minDist = 200; }

            @Override
            public void run() {
                drive.gyroDrive(0.1,0, initialHeading + Math.toRadians(90 * X_MOD));
            }
        ...
        // Align with cone stack using front distance sensor but LEFT
        motions.add(new Motion() {
            @Override
            public boolean isEnd() {
                dist = drive.distanceSensorFront.getDistance(DistanceUnit.CM);
                return dist < minDist + 0.6 || runtime.seconds() > 2;
            }

            @Override
            public void init() { runtime.reset(); }
        ...
        motions.add(new Motion() {
            @Override
            public boolean isEnd() {
                return drive.distanceSensorFront.getDistance(DistanceUnit.CM) < 23;
            }
        ...
        motions.add(new Motion() {
            ...
            @Override
            public void cleanup() {
                drive.slideMotor.setTargetPosition(Constants.HIGH_ARM_POS + 100);
                drive.slideMotor.setMode(DcMotor.RunMode.RUN_TO_POSITION);
                drive.slideMotor.setVelocity(Constants.ARM_TPS);
        ...
        motions.add(new Motion() {
            ...
            @Override
            public void run() {
                drive.gyroDrive(-0.1,0, initialHeading);
            }
            ...
            public void init() {
                ...
                drive.followTrajectorySequence(drive.trajectorySequenceBuilder(lastStartPose)
                        .lineTo(new Vector2d(lastStartPose.getX(),startY + (POLE_DIST_CM - dist)/2.54),
                                JebRunner.getVelocityConstraint(0.5 * Constants.MAX_VEL, Constants.MAX_ANG_VEL, Constants.TRACK_WIDTH), JebRunner.getAccelerationConstraint(Constants.MAX_ACCEL))
                        .build()
                );
            }
        ...
        motions.add(new Motion() {
            ...
            public void init() {
                ...
                drive.followTrajectorySequence(drive.trajectorySequenceBuilder(lastStartPose)
                        ...
                        .splineToLinearHeading(new Pose2d((parkTarget == 0 ? -56 * X_MOD : (parkTarget == 2 ? -12 * X_MOD : -36 * X_MOD)),
